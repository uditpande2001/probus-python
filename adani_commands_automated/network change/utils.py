...
def is_network_address_channel_channel_successful(payload):
    cancel_resp = unpack('<B', payload[0:1])[0]
    begin_resp = unpack('<B', payload[2:3])[0]
    network_address_change = unpack('<B', payload[4:5])[0]
    network_channel_change = unpack('<B', payload[11:12])[0]

    return cancel_resp == 0x84 and begin_resp == 0x81 and network_address_change == 0x8d and network_channel_change == 0x8d
...
def convert_to_network_address(network_address):
    hex_network_address_str = ''.join(list(sliced("{:06x}".format(int(network_address)), 2)))
    return binascii.hexlify(bytes(bytearray.fromhex(hex_network_address_str)[::-1])).decode()

def convert_to_network_channel(network_channel):
    return  "{:02x}".format(int(network_channel))
...
def build_change_node_address_command(node_address, activation_delay):
    return "040001000D060100" + convert_node_address(node_address) + "0300" + add_activation_delay(activation_delay)

def build_change_node_network_address_and_channel(network_address, network_channel,activation_delay):
    return "040001000D050200" + convert_to_network_address(network_address) + "0D030300" + convert_to_network_channel(network_channel) + "0300" + add_activation_delay(activation_delay)
