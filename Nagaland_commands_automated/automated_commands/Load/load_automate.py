...
def get_db_data():
    try:
        ...
        load_query = f"""                           
                            ...
                            SELECT
                                meter_number ,
                                count(DISTINCT date_time) AS load_count
                            FROM
                                meter_profile_data mpd
                            WHERE
                                date_time >= '{midnight_datetime}'
                            ...
                            diag_data AS (
                                            SELECT
                                                req_nodes.node_id,
                                                mapping_data.meter_number,
                                                req_nodes.latest_diag
                                            FROM(
                                SELECT
                                    node_id ,
                                    max(server_time) AS latest_diag
                                FROM
                                    rf_diag rd
                                WHERE
                                    server_time >= '{current_datetime - timedelta(minutes=45)}'
                                GROUP BY
                                    node_id) req_nodes
                                LEFT JOIN 
                                (	SELECT *
                                    FROM
                                    meter_mapping mm) mapping_data ON req_nodes.node_id = mapping_data.node_id)
                                                       
                                SELECT *
                                FROM
                                    load_data
                                LEFT JOIN diag_data ON
                                    load_data.meter_number = diag_data.meter_number
                                    WHERE load_data.load_count <75
                           """
...
def send_command():
    ...
    file = open('Load_Sent_nodes.csv', 'w', newline='')
    for node_array in nodes_batch_array:
        for node in node_array:
            ...
            file.write(str(node))
            file.write("\n")
            print(f'load command send to {node}  remaining nodes = {num_elements} ', flush=True)
