...
def getCounters(node_id):
    try:

        url = base_url + "config/getCounters"
        headers = {"Authorization": res_token}
        params = {'nodeId': node_id}
        ...
        if response.status_code == 200:
            ...
            with open('Getcounters.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([node_id, response_txt])

            logging.info(response)
            logging.info(response.text)
        else:
            response_txt = response.text
            with open('Getcounters_error.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([node_id, response_txt])
