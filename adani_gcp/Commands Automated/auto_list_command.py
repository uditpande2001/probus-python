...
from master.master_file import list_command_array
...
RELATIVE_PATH = "List_Command_Response"
...
def get_db_data():
    ...
    try:
        ...
        for result in results:
            if result[1] in list_command_array:
    ...
    try:
        ...
        for array in batch_array:
            for node_id in array:
                count += 1
                print(f"list command sent to {node_id} count = {count} nodes left = {num_elements -count}")
...
def cr_consumer():
    ...
    for message in consumer:
        ...
        if int(node_id) in list_command_array:
            ...
            if command_type == 'CONNECT-DISCONNECT':
                print(message.value)
...
if __name__ == '__main__':
    try:
        ...
        if not os.path.exists(file_path):
            os.mkdir(file_path)
        with open(file_path + '\\' + f"List_command_test_file_{timestamp}.csv", 'w', newline='') as csvfile:
